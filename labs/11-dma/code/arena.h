...
void *arena_alloc_aligned(arena_t *a, unsigned nbytes, unsigned alignment) {
    assert(nbytes);
    assert(nbytes % 8 == 0);

    // XXX: we waste the alignment memory.  aiya.
    //  really should migrate to the k&r allocator.
    unsigned rem = (uint32_t)a->cur % alignment;
    if(rem) {
        arena_alloc(a, rem);
        rem = (uint32_t)a->cur % alignment;
        assert(!rem);
    }

    return arena_alloc(a,nbytes);
}
